...
export class MarkdownReporter {
	generate(result: AnalysisResult): string {
		...
		lines.push(
			`- **期間**: ${filteredResult.startDate} ~ ${filteredResult.endDate}`,
		);
		lines.push("");
	...
	private filterEntityMoves(entities: EntityChange[]): EntityChange[] {
		...
		return [
			...entities.filter(
				(e) => !movedClassNames.has(e.className) || e.changeType === "moved",
			),
			...movedEntities,
		];
	}
	...
	private filterDTOMoves(dtos: DTOChange[]): DTOChange[] {
		...
		return [
			...dtos.filter(
				(d) => !movedClassNames.has(d.className) || d.changeType === "moved",
			),
			...movedDTOs,
		];
	}
	...
	private filterEnumMoves(enums: EnumChange[]): EnumChange[] {
		...
		return [
			...enums.filter(
				(e) => !movedEnumNames.has(e.enumName) || e.changeType === "moved",
			),
			...movedEnums,
		];
	}

	private filterInterfaceMoves(
		...
		return [
			...interfaces.filter(
				(i) =>
					!movedInterfaceNames.has(i.interfaceName) || i.changeType === "moved",
			),
			...movedInterfaces,
		];
	}
	...
	private generateSummary(result: AnalysisResult): string[] {
		...
		if (entitiesToShow.length > 0) {
			...
			for (const entity of entitiesToShow) {
				...
				if (entity.changeType === "moved") {
					...
					const oldDir = entity.oldFile
						? this.getDirectory(entity.oldFile)
						: "";
					...
					if (addedColumns.length > 0) {
						changes.push(
							`+${addedColumns.length} columns: ${addedColumns.join(", ")}`,
						);
					}
					if (deletedColumns.length > 0) {
						changes.push(
							`-${deletedColumns.length} columns: ${deletedColumns.join(", ")}`,
						);
					}
					if (modifiedColumns.length > 0) {
						changes.push(
							`~${modifiedColumns.length} columns: ${modifiedColumns.join(", ")}`,
						);
					}
					if (addedRelations.length > 0) {
						changes.push(
							`+${addedRelations.length} relations: ${addedRelations.join(", ")}`,
						);
					}
					if (deletedRelations.length > 0) {
						changes.push(
							`-${deletedRelations.length} relations: ${deletedRelations.join(", ")}`,
						);
					}
		...
		if (dtosToShow.length > 0) {
			...
			for (const dto of dtosToShow) {
				...
				if (dto.changeType === "moved") {
					...
					if (addedProps.length > 0) {
						changes.push(
							`+${addedProps.length} props: ${addedProps.join(", ")}`,
						);
					}
					if (deletedProps.length > 0) {
						changes.push(
							`-${deletedProps.length} props: ${deletedProps.join(", ")}`,
						);
					}
					if (modifiedProps.length > 0) {
						changes.push(
							`~${modifiedProps.length} props: ${modifiedProps.join(", ")}`,
						);
					}
		...
		if (enumsToShow.length > 0) {
			...
			for (const enumChange of enumsToShow) {
				...
				if (enumChange.changeType === "moved") {
					const oldDir = enumChange.oldFile
						? this.getDirectory(enumChange.oldFile)
						: "";
					...
					if (addedMembers.length > 0) {
						changes.push(
							`+${addedMembers.length} members: ${addedMembers.join(", ")}`,
						);
					}
					if (deletedMembers.length > 0) {
						changes.push(
							`-${deletedMembers.length} members: ${deletedMembers.join(", ")}`,
						);
					}
					if (modifiedMembers.length > 0) {
						changes.push(
							`~${modifiedMembers.length} members: ${modifiedMembers.join(", ")}`,
						);
					}
		...
		const interfacesToShow = result.interfaces.filter((interfaceChange) => {
			...
			const modifiedProps =
				this.getModifiedInterfaceProperties(interfaceChange);
			const extendsChanged =
				interfaceChange.extendsInterfaces.before.join(",") !==
		...
		if (interfacesToShow.length > 0) {
			...
			for (const interfaceChange of interfacesToShow) {
				...
				if (interfaceChange.changeType === "moved") {
					const oldDir = interfaceChange.oldFile
						? this.getDirectory(interfaceChange.oldFile)
						: "";
		...
		if (endpointChanges.length > 0) {
			...
			for (const change of endpointChanges) {
				lines.push(
					`| ${change.symbol} | ${change.method} | \`${change.path}\` |`,
				);
			}
			lines.push("");
		}
		...
		const providersToShow = result.providers.filter(
			(p) => p.changeType !== "deleted",
		);
		...
		const middlewaresToShow = result.middlewares.filter(
			(m) => m.changeType !== "deleted",
		);
	...
	private generateEntityDetails(entities: EntityChange[]): string[] {
		...
		const entitiesToShow = entities.filter((entity) => {
			...
			const allColumns = this.mergeColumns(
				entity.columns.before,
				entity.columns.after,
			);
			const changedColumns = allColumns.filter(
				(col) => col.changeType !== "unchanged",
			);
			const allRelations = this.mergeRelations(
				entity.relations?.before ?? [],
				entity.relations?.after ?? [],
			);
			const changedRelations = allRelations.filter(
				(rel) => rel.changeType !== "unchanged",
			);
						...
						rel.changeType === "added"
							...
							: rel.changeType === "deleted"
								...
								: rel.changeType === "modified"
									...
									: "";
					const relationType =
						rel.after?.relationType ?? rel.before?.relationType ?? "";
					const target =
						rel.after?.targetEntity ?? rel.before?.targetEntity ?? "";
					lines.push(
						`| ${symbol} | ${rel.name} | ${relationType} | ${target} |`,
					);
				}
				lines.push("");
			}
	...
	private generateDTODetails(dtos: DTOChange[]): string[] {
		...
		const dtosToShow = dtos.filter((dto) => {
			...
			const allProperties = this.mergeProperties(
				dto.properties.before,
				dto.properties.after,
			);
			const changedProperties = allProperties.filter(
				(prop) => prop.changeType !== "unchanged",
			);
		...
		for (const dto of dtosToShow) {
			...
			if (allProperties.length > 0) {
				...
				for (const prop of allProperties) {
					...
					const decorators =
						prop.after?.decorators.join(", ") ??
						prop.before?.decorators.join(", ") ??
						"";
	...
	private generateEnumDetails(enums: EnumChange[]): string[] {
		...
		const enumsToShow = enums.filter((enumChange) => {
			...
			if (enumChange.changeType === "added") return true;
			if (enumChange.changeType === "moved") return true;
			const allMembers = this.mergeEnumMembers(
				enumChange.members.before,
				enumChange.members.after,
			);
			const changedMembers = allMembers.filter(
				(member) => member.changeType !== "unchanged",
			);
		...
		for (const enumChange of enumsToShow) {
			...
			if (enumChange.changeType === "moved" && enumChange.oldFile) {
				lines.push(
					`> Moved: \`${enumChange.oldFile}\` → \`${enumChange.file}\``,
				);
				lines.push("");
			}
	...
	private generateInterfaceDetails(interfaces: InterfaceChange[]): string[] {
		...
		for (const interfaceChange of interfacesToShow) {
			const fileName =
				interfaceChange.file.split("/").pop() ?? interfaceChange.file;
			lines.push(`### ${fileName}`);

			// 移動の場合、移動元パスを表示
			if (interfaceChange.changeType === "moved" && interfaceChange.oldFile) {
				lines.push(
					`> Moved: \`${interfaceChange.oldFile}\` → \`${interfaceChange.file}\``,
				);
				lines.push("");
			}
			...
			if (allProperties.length > 0) {
				...
				for (const prop of allProperties) {
					...
					lines.push(
						`| ${symbol} | ${prop.name} | ${type} | ${modifierText} |`,
					);
				}
				lines.push("");
			}
	...
	private generateControllerDetails(controllers: ControllerChange[]): string[] {
		...
		const controllersToShow = controllers.filter((controller) => {
			...
			const allEndpoints = this.mergeEndpoints(
				controller.endpoints.before,
				controller.endpoints.after,
			);
			const changedEndpoints = allEndpoints.filter(
				(ep) => ep.changeType !== "unchanged",
			);
			// 全て削除の場合も出さない
			const hasAddedOrModified = changedEndpoints.some(
				(ep) => ep.changeType === "added",
			);
	...
	private generateModuleDetails(modules: ModuleChange[]): string[] {
		...
		for (const mod of modulesToShow) {
			...
			for (const change of configChanges) {
				lines.push(
					`| ${change.symbol} | ${change.key} | ${change.before} | ${change.after} |`,
				);
			}
	...
	private generateProviderDetails(providers: ProviderChange[]): string[] {
		...
		for (const provider of providersToShow) {
			const symbol = this.getChangeSymbol(provider.changeType);
			const fileName =
				provider.info.file.split("/").pop() ?? provider.info.file;
	...
	private generateMiddlewareDetails(middlewares: MiddlewareChange[]): string[] {
		...
		const middlewaresToShow = middlewares.filter(
			(m) => m.changeType !== "deleted",
		);
		...
		for (const mw of middlewaresToShow) {
			...
			const typeLabel = this.getMiddlewareTypeLabel(mw.info.type);
			lines.push(
				`| ${symbol} | ${fileName} | ${typeLabel} | ${mw.info.className} |`,
			);
		}
	...
	private getModifiedColumns(entity: EntityChange): string[] {
		...
		for (const after of entity.columns.after) {
			...
			if (
				before &&
				(before.type !== after.type || before.nullable !== after.nullable)
			) {
	...
	private getModifiedMembers(enumChange: EnumChange): string[] {
		const beforeMap = new Map(
			enumChange.members.before.map((m) => [m.name, m]),
		);
		const result: string[] = [];
	...
	private getAddedInterfaceProperties(
		interfaceChange: InterfaceChange,
	): string[] {
		const beforeNames = new Set(
			interfaceChange.properties.before.map((p) => p.name),
		);
	...
	private getDeletedInterfaceProperties(
		interfaceChange: InterfaceChange,
	): string[] {
		const afterNames = new Set(
			interfaceChange.properties.after.map((p) => p.name),
		);
	...
	private getModifiedInterfaceProperties(
		interfaceChange: InterfaceChange,
	): string[] {
		const beforeMap = new Map(
			interfaceChange.properties.before.map((p) => [p.name, p]),
		);
		const result: string[] = [];
	...
	private getEndpointChanges(controllers: ControllerChange[]): Array<{
		...
		for (const controller of controllers) {
			const merged = this.mergeEndpoints(
				controller.endpoints.before,
				controller.endpoints.after,
			);
	...
	private getModuleChangeSummary(mod: ModuleChange): string {
		...
		const addedImports = this.getAddedItems(
			mod.config.before.imports,
			mod.config.after.imports,
		);
		if (addedImports.length > 0) {
			parts.push(`+imports: ${addedImports.join(", ")}`);
		}

		const addedProviders = this.getAddedItems(
			mod.config.before.providers,
			mod.config.after.providers,
		);
		if (addedProviders.length > 0) {
			parts.push(`+providers: ${addedProviders.join(", ")}`);
		}

		const addedExports = this.getAddedItems(
			mod.config.before.exports,
			mod.config.after.exports,
		);
		if (addedExports.length > 0) {
			parts.push(`+exports: ${addedExports.join(", ")}`);
		}

		const addedControllers = this.getAddedItems(
			mod.config.before.controllers,
			mod.config.after.controllers,
		);
	...
	private getAddedItems(
		before: Array<{ name: string }>,
		after: Array<{ name: string }>,
	): string[] {

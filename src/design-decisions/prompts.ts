...
**補足セクション:**
${hasEntityChanges ? "4" : "3"}. **その他補足**: 以下を含むが、上記${hasEntityChanges ? "3" : "2"}セクションで既に言及した内容は除外する。**カテゴリ見出しを使用せず**、統一された箇条書きで記載する。**1ドメインあたり最大4項目**を目安とし、超える場合は重要度の高いものを優先する。
   - 確定したビジネスルール（バリデーション・制約・ロジック）
   - 新たな概念・用語（ドメイン用語・状態名・クラス名）
   - **Nest.jsベストプラクティス違反**: 発見した場合は必ず明記する
   - **以下は記載しない**（実装の詳細であり設計判断ではないため）: 単なる新規メソッド・関数の追加、依存パッケージ追加・バージョン変更、環境変数追加（主要セクションで既出のものの再掲）、ファイル名・ディレクトリ名の追加・移動・削除、主要セクションで既に言及済みの固有名詞（クラス名・env名・enum名・migration名など）
...
**PRリンクの記載ルール:**
- ドメイン内で参照するPRが **1〜2件** の場合: ドメイン見出し直下に \`**関連PR:** [#番号](url)[, [#番号](url)]\` の形式で1行に集約し、各bulletからは省略する
- ドメイン内で参照するPRが **3件以上** の場合のみ: 各bullet末尾に \`*← [#番号](url)*\` を付与する
- 同じPRリンクを同一ドメイン内で5回以上反復しないこと

### [ドメイン名]
**関連PR:** [#番号](url)　（※参照PRが1〜2件のドメインのみこの行を出力。3件以上ならこの行は出さず、各bulletに付与する）

#### 責務・境界の変化
- （どの処理がどこからどこへ移ったか、または新たにどこに置かれたか）　*← [#PR番号](url)*　（※参照PRが3件以上のドメインのみ各bullet末尾に付与）
- ...

#### この設計が前提としていること
- （暗黙の仮定・現時点での制限・将来変わりうる前提を読み取れる範囲で記述。**体言止めまたは短文を推奨し、「〜を前提としている」の語尾を機械的に反復しない**）
- 例: 「S3バケット名は環境変数で指定（環境ごとに設定要）」
- 例: 「ファイル種別判定はファイル名ベース → 命名規則の遵守が必須」
- 例: 「\`hasCost\` フィルタは cost / costWithMargin のいずれかが設定済みのときのみ true」

${
	hasEntityChanges
		? `（↓このセクションは、当該ドメインに該当するEntity変更が**存在する場合のみ**出力すること。該当する変更がなければセクションヘッダごと省略し、「変更なし」等のプレースホルダ記載は禁止）
...
**重要 (per-domain ルール):** 「データ構造の変化」セクションは、**当該ドメインに該当するEntity変更（\`*.entity.ts\` の追加・削除・変更）が存在する場合のみ**出力してください。該当する変更がないドメインでは、セクションヘッダ（\`#### データ構造の変化\`）ごと省略してください。「変更なし」「Entity変更なし」「（このドメインではEntity変更なし）」等のプレースホルダ記載は禁止です。
...
以下の内容を**カテゴリ見出しなしの統一された箇条書き**で記載してください。**1ドメインあたり最大4項目**を目安とし、超える場合は重要度の高いものを優先してください：
...
以下は実装の詳細であり、それ自体が設計判断でない限り「その他補足」に**記載しないでください**:

- 単なる新規メソッド・関数の追加（例: 「ServiceにXメソッドが追加された」）
- 依存パッケージの追加・バージョン変更（例: 「multer 2.1.1 が追加された」）
- 環境変数の追加（主要セクションで既に言及されている場合は再掲しない）
- ファイル名・ディレクトリ名の追加・移動・削除
- 主要セクション（責務・境界 / 前提 / データ構造）で既に言及した固有名詞（クラス名・env名・enum名・migration名など）の再掲

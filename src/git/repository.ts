...
export class GitRepository {
	...
	async getChangedFilesSince(
		...
		for (const file of diffSummary.files) {
			const changeType: FileChange["changeType"] = "modified";
	...
	async getOldestCommitHashSince(
		days: number,
		branch?: string,
	): Promise<string | null> {
		const since = new Date();
	...
	async getDiffFiles(
		...
		for (const [from, to] of renameMap.entries()) {
			const existedBefore = oldestHash
				? await this.fileExistsAt(from, `${oldestHash}^`)
				: false;
		...
		for (const file of fileSet) {
			...
			const existedBefore = oldestHash
				? await this.fileExistsAt(file, `${oldestHash}^`)
				: false;

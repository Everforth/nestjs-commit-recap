...
export class PRFetcher {
  ...
  async fetchMergedPRs(days: number): Promise<PRInfo[]> {
    ...
    try {
      ...
      const result = execSync(
        `gh pr list --state merged --search "merged:>=${sinceStr}" --json number,title,url,mergedAt --limit 100`,
        {
          cwd: this.repoPath,
          encoding: 'utf-8',
  ...
  async getPRsForCommits(commitHashes: string[]): Promise<Map<string, PRInfo>> {
    ...
    for (const hash of commitHashes) {
      try {
        const result = execSync(
          `gh pr list --state merged --search "${hash}" --json number,title,url,mergedAt --limit 1`,
          {
            cwd: this.repoPath,
            encoding: 'utf-8',
            stdio: ['pipe', 'pipe', 'pipe'],
  ...
  async getPRsForFiles(files: string[], days: number): Promise<Map<string, PRInfo[]>> {

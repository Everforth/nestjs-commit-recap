...
import { Project, SourceFile, ClassDeclaration, Decorator, Node, SyntaxKind } from 'ts-morph';
...
export abstract class BaseAnalyzer {
  ...
  constructor(repo: GitRepository, prFetcher: PRFetcher, options: AnalyzerOptions) {
    this.repo = repo;
   ...
   */
  protected findClassWithDecorator(sourceFile: SourceFile, decoratorName: string): ClassDeclaration | undefined {
   ...
   */
  protected findAllClassesWithDecorator(sourceFile: SourceFile, decoratorName: string): ClassDeclaration[] {
    const classes = sourceFile.getClasses();
   ...
   */
  protected getDecorator(node: ClassDeclaration, decoratorName: string): Decorator | undefined {
   ...
   */
  protected getDecoratorObjectArgument(decorator: Decorator): Map<string, string> | null {
    const args = decorator.getArguments();
    if (args.length === 0) return null;
   ...
   */
  protected classImplements(cls: ClassDeclaration, interfaceName: string): boolean {
  ...
  protected async getFileContent(
    ...
    if (position === 'before') {
      return this.repo.getFileContentBefore(file, this.options.days, this.options.branch);
    }
  ...
  protected extractClassName(content: string, decoratorPattern: RegExp): string | null {

...
import {
	classifyFile,
	fileTypeToMiddlewareType,
	isMiddlewareTypeFile,
...
export class MiddlewareAnalyzer extends BaseAnalyzer {
	async analyze(): Promise<MiddlewareChange[]> {
		...
		const middlewareFiles = [...added, ...deleted, ...modified].filter(
			isMiddlewareTypeFile,
		);
		...
		for (const file of middlewareFiles) {
			...
			if (
				!this.isMiddlewareContent(beforeContent, middlewareType) &&
	...
	private extractMiddlewareClassName(
		content: string | null,
		type: MiddlewareType,
	): string | null {
		...
		try {
			...
			const targetClass = classes.find((cls) =>
				this.classImplements(cls, interfaceName),
			);
